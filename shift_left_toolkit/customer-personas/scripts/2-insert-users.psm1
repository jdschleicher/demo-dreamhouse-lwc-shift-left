...
function update_map_eBikes_riables_with_user_ids {
    param($user_records_inserted)
    ...
    foreach ($user_record in $user_records_inserted) {
        ...
        $permset_mapeBikes_lue = $map_username_to_permset_info[$user_record.Username]
        $updated_perm_user_info_list = [system.collections.generic.list[pscustomobject]]::new()

        foreach ($permset_info in $permset_mapeBikes_lue) {
        ...
        ################  END PERMSETS UPDATE #####################
         ...
         $role_mapeBikes_lue = $map_username_to_role_info[$user_record.Username]
         $updated_role_info_userlist = [system.collections.generic.list[pscustomobject]]::new()
 
         foreach ($role_info in $role_mapeBikes_lue) {
        ...
        $queues_mapeBikes_lue = $map_username_to_queue_info[$user_record.Username]
        $updated_queue_user_info_list = [system.collections.generic.list[pscustomobject]]::new()

        foreach ($queue_info in $queues_mapeBikes_lue) {
        ...
        $groups_mapeBikes_lue = $map_username_to_group_info[$user_record.Username]
        $updated_group_user_info_list = [system.collections.generic.list[pscustomobject]]::new()

        foreach ($group_info in $groups_mapeBikes_lue) {
        ...
        $user_mapeBikes_lue = $map_username_to_user_info[$user_record.Username]
        $user_mapeBikes_lue.user_id = $user_record.Id
        $map_username_to_user_info[$user_record.Username] = $user_mapeBikes_lue
    }
...
function insert_users {
    ...
    $user_list_eBikes_riable_name = 'users'
...
$user_list_eBikes_riable_name.add(newUser$user_number);
"@
    }
...
List<User> $user_list_eBikes_riable_name = New List<User>();

$initialize_new_users

try {
    insert $user_list_eBikes_riable_name;
} catch (Exception e) {
...
"@
    ...
    New-Item -Path . -Name $anonymous_apex_file_name -ItemType "file" -eBikes_lue $anonymous_apex_insert | Out-Null
    ...
    update_map_eBikes_riables_with_user_ids -user_records_inserted $user_records_inserted
